...
- **链接有效性检测**: 转存/返回前逐一检测原始链接是否有效，过滤失效链接，支持3个检测源随机轮换降级（自建 PanCheck / PanSou 内置 / 116818 第三方）
...
用户搜索 "流浪地球" (pan_type=quark, limit=3)
  ...
  ├─ L1 搜索缓存命中 → 直接返回（毫秒级）
  │
  ├─ L1 未命中 → 请求 PanSou → 按时间降序逐一处理
  │     │
  │     ├─ L3/DB 资源缓存命中 → 直接计入结果（毫秒级，无需检测）
  │     │
  │     └─ 均未命中 → 链接有效性检测（随机轮换3个检测源）
  │           ├─ 失效 → 跳过，取下一条
  │           └─ 有效/不确定 → 获取 L2 分布式锁
  │                 ├─ 有账号 → 账号调度 → 转存 → 分享 → 写 DB + L3 缓存
  │                 └─ 无账号 → 直接返回原始链接
  │     （凑够 limit 条即停止）
  │
  └─ 所有检测源失败 → 放行转存（不因检测失败阻塞流程）
```
